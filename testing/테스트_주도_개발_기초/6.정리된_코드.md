...
## 리팩토링
- 원래 코드를 다른 코드로 바꾸면 무조건 리팩토링? 아니다
- 의미를 유지하며 코드베이스를 정리
...
- index.js 2차
    ```
        ...
        function compacthWithSpaces(source) {
            return source.indexOf("  ") < 0 : source ? compactWithSpaces(source.replace("  ", " "))
        }

        function normalizeWitheSpace (source) {
...
- index.js 3차
    ```
        function refineText(source, options) {
                return [normalizeWithWhiteSpaces, compactWhiteSpaces, maskBannedWords].reduce(((value, filter) => filter(value, options), source)
            }
...
- index.js 5차
    ```
        function refineText(source, options) {
                return [normalizeWitheWhiteSpaces, compacthWithSpaces, maskBannedWords].reduce(((value, filter) => filter(value, options), source)
            }
