...
- 예제)
    ...
    ```
        ...
        // input$에 3,1,4,7,5,7,8,2 입력 시 true
        // 그 외 false 입력
    ```
...
- 발행물이 없이 Stream이 complete 시, 기본값으로 발행시킴.
- 예제)
...
- false 시에 대한 스트림이 주어지지 않으면, false 시 바로 complete
  - 즉 false 시, 빈 Stream을 반환함.
- 예제)
...
- 아무 값도 반환하지 않고 출력
- 활용
  - 조건에 따라서 Stream으로 처리해야 하지만, 값을 반환하고 싶지 않을 때
- 예제)
...
- 모든 observer에 대해서, Side Effect 연산자(tap 등)가 공유 됨.
  - 중간 합류 시 또한 성립됨.
- 예제)
