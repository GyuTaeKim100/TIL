...
1. 목적과 의도를 파악한다.
2. 최대한 많은 요구사항을 수집한다.
3. 요구사항이 자주 변경되는 여부를 파악한다.
4. 러닝 커브 최소화하기 위한 방안 (통일성 등...)
...
- 제약보다 우선순위가 높은 경우는, 요구사항의 추가 또는 변경에 대응하기 위해서이다.
  - 변화하는 제품은 Iterator를 거치며 고객의 요구사항을 반영하거나,
  - 기존의 결정을 번복하고,
  - 다른 결정을 내리기도 한다.
  - 변화하는 제품의 재료가 되는 디자인 시스템은 변화의 폭이 좁을 수 있으나,
  - 모든 요구사항에 최대한 유연하게 대응해야 한다.
  - 발전하는 디자인 시스템이라고 가정하고,
  - 현재 결정은 항상 변할 수 있다는 전제를 견지하면서,
  - 시스템에서 보장해야 하는 부분과 아닌 부분을 분리해야 한다.

- 형태를 가지면서 유연함을 추구하면,
  - 일관성을 위한 제약은 적다.
  - 단, 일관성에 대한 책임을 일부 사용처에 위임하는 경우가 발생

- 유연함과 제약은 상충 관계이다.
  - 두 조건을 모두 달성하는 이상적인 방법은,
  - 즉 디자인 케이스를 모두 사전에 예측하여 변화가 필요한 부분과,
...
- Compound Component 방식
  ...
  - 코드 예시, Select Box)
    ...
    - ```
      ...
      - 같은 소비자가 전달한 prop을 전달하는 곳과, 필요한 곳은 일치하지 않아도 상위 Context를 통해 필요한 값을 사용할 수 있으면서 인터페이스를 직관적으로 유지할 수 ㅣㅆ다.
...
- 무조건 제약보다는 유연함을 추구한다고, 소비자의 실수에 책임을 지지 않는다는 의미는 아니다.
