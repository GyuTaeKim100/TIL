 ...
 - 종류
  ...
  3. **202 Accepted**
    - 요청이 접수되었으나 처리가 완료되지 않았음
        - 예시
          - 배치 처리) 요청 접수 후 1시간 뒤에 배치 프로세스가 요청을 처리함
  4. **204 No Content*
...
## 3xx (Redirection)
    ...
    2. 일시 리다이렉션
        ...
        - 종류
          ...
          - PRG 패턴 : Post/Redirect/Get
            - POST로 주문후에 새로 고침으로 인한 중복 주문 방지 (클라이언트 차원에서 블록 처리, 그 외 별도로 서버 차원에서 블록 처리 필요)
  ...
  - 기타 status 코드 종류
    ...
    - **304 Not Modified**
      - 캐시를 목적으로 사용
      - 클라이언트에게 리소스가 수정되지 않았음을 알려준다. 따라서 클라이언트는 로컬에 저장된 캐시를 재사용한다. (캐시로 리다이렉트 한다.)
...
## 401 Unauthorized
 ...
 - 참고
    ...
    2. 인가(Authorization): 권한부여 (특정 리소스에 접근할 수 있는 권한, 인증 후 인가가 존재)
