...
## HTTP 메서드의 속성
 - ![115.png](./img/15.png)
 1. 안전 (Safe Methods)
    - 호출 시 리소스 변경에 영향을 주지 않음 (예로 GET에 의한 단순 조회)
    - 의문
        - 반복 호출시, 로그 쌓여서 장애 발생 시 안전하지 않은 것 아님?
           - 답변 : 안전은 해당 요청에 대한 리소스에 한정
 2. 멱등 (Idempotent Methods)
    - f(f(x)) = f(x)
    - 한 번 호출한 경우와 n번 호출한 경우에 대해서 모든 결과가 똑같다
    - 멱등 메서드
        1. GET : 한 번 조회, n번 조회 시 *같은 결과* 조회
        2. PUT : 결과를 대체, *같은 요청을 여러번* 하는 경우에 대해서 최종 결과는 같음
        3. DELETE : 결과를 삭제, *같은 요청을 여러번* 하는 경우에 대해서 삭제된 결과는 같음
        4. **POST** : 멱등 성립하지 않음. 두번 호출 시 같은 *결제가 중복해서 발생할 가능성* 존재
        - 활용
            1. 자동 복구 메커니즘
                - 클라이언트가 DELETE 요청 시 요청에 대한 응답이 안 온 경우, DELETE 요청을 다시 한다 
            2. 서버가 TIMEOUT 등으로 정상 응답에 실패한 경우, 클라이언트에서 같은 요청에 대해서 재요청 시도가 가능한 여부에 대한 판단 근거
        - 의문
            - Q: 재요청 중간에 다른 곳에서 리소스를 변경 시?
                1. 사용자1: GET -> username:A, age: 20
                2. 사용자2: PUT -> username:A, age: 30
                3. 사용자1: GET -> username:A, age: 30 // 사용자 2의 영향에 의해 바뀐 데이터를 조회하게 된다
            - A: *멱등은 외부 요인으로 중간에 리소스가 변경되는 것은 고려하지 않음*
 3. 캐시가능 (Cacheable Methods)
    ...
    - 가능 Method
        ...
        - 캐시 사용을 위해서는 리소스에 대한 catch key가 일치해야 함.
