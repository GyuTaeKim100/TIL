 ...
 - ![69.png](./img/69.png)
     - 의문
         - cache-control과 Last-Modified는 클라이언트 요청 헤더에서만 사용이 가능? 또는 서버 응답에서도 사용 가능?
           - 답: 서버에서 캐시 대상 데이터인 경우 Last-Modifed 를 응답 헤더에 포함한다. 그 후 클라이언트는 응답받은 Last-Modified 값을 활용한 If-Modified-Since를 사용하는 입장이다.
 - ![70.png](./img/70.png) 
 - ![71.png](./img/71.png)
 - ![72.png](./img/72.png)
 - ![73.png](./img/73.png)
 ...
 - Cache-Control: public
   ...
   - 의문
     - public에 데이터 캐시 처리하는 과정은? 서버에서 프록시 서버로 캐시 대상 데이터를 밀어 넣은 후 redirect 응답을 통해 프록시 서버의 캐시 데이터를 재 다운로드 처리를 할까? 근데 데이터를 밀어넣는 시간이 오래 걸리면 클라이언트 요청에 대한 응답이 느려질 수도 있다.
       - 답: 클라이언트가 프록시 캐시에게 요청 후 프록시 캐시가 캐시된 데이터가 없거나 캐시 유효성 검사가 필요한 경우 원서버에게 추가 요청함
...
## 캐시 무효화
 - 확실한 캐시 무효화 응답 
   - 추가
     ...
     - pragma: no-cache (HTTP 1.0 하위 호환 - 과거 브라우저)
 - 주석
   - 캐시를 적용 안해도 웹 브라우저가 GET 요청에 대해 임의로 휴리스틱하게 캐시 처리를 임의로 함. 그러므로 cache를 하지 않아야 되는 경우 반드시 캐시 무효화 적용 (예: 사용자 통장 잔고)
...
## no-cache vs must-revalidate
 ...
 - ![89.png](./img/89.png)
   - 의문
     - 원서버가 요청에 대한 응답을 프록시 캐시에 어떤 방식을 통해서 전달하고 프록시 캐시는 어떤 방식을 통해서 클라이언트에게 응답을 전달하는가? 
 - 만약 원서버에 접근 불가한 경우1
   - ![90.png](./img/90.png) 
 - 만약 원서버에 접근 불가한 경우2
   - ![91.png](./img/91.png)  
