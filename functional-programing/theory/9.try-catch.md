...
## Try Type을 이용한 try-catch와 Try 타입의 트레이드 오프 판단
 - 목적
     - 베이스 예제에서 상품 별 수량이 잘 못 지정된 경우 try-catct와 Try 타입의 트레이드 오프를 판단한다.
 - 추가 요구사항
     - 한 번에 구매할 수 있는 갯수 제한
        ...
        const stockItem = (item: ParsedItem): string => {
          ...
          const item = (item: Item) : string => {
              Try {
                ...
                // Option에서는 값이 존재할 때만 어떤 계산을 실행하기 위해서 map을 사용해왔다.
                // Try에서의 map은 어떤 동작을 해야 할까?
                // Try의 map은 실패했을 때는 자신을 리턴하고, 성공일 때만 인자로 전달된 함수를 자신의 데이터에 적용하고 결과를 리턴하면 된다.
                // 이 과정에서 성공과 실패의 여부는 바뀌지 않는다.
...
- 예제 2 - 7 / Type 도입 / Try Type을 통한 분기문 처리 / Try의 map 함수 적용4
    - Try의 map
     ...
     - ```
          ...
          //       parseInt를 통해 이전에 적용된 parseInt를 반복해야 하는지 의문이 들 수 있다.
          //       다음 섹션에서 이 의문을 해결하자.
          const totalCalculator = (list: ArrayItem, getValue: (item: Item) => number) : number {
              return list
                .filter((item) => {
...
- 목적
  - 이전 섹션에서는 item 함수에서 parseItem을 하고 있는데, 이는 같은 레벨(?)의 함수인 totalCaluator에서도 parseInt를 사용하게 하는 반복의 문제가 존재헀다.
  - 이를 해결해보도록 한다.
- 베이스 예제 1
    - ```
        type Success<R> = {
   ...
   - ```
        ...
        export const KeepSuccess = <E, R>(tas: Array<T, R>>) => Array<R> => {
            const ret = tas.map((ta)=> {
                ...
                // 에러 발생 이유 (잘 이해가 안됨)
                //  - ta.error를 리턴 할 수 없다. 함수 리턴 타입은 Array<R>이다.
                //  - else 구문을 삭제한다고 해도 에러가 여전하다.
                // 함수 리턴 타입이 자동으로 undefined가 타입 추론 되고 있다. 
                // 즉 에러일 때는 undefined를 리턴하는 것과 같다.
                // 이런 문제 때문에 map 함수로 keepSuccess를 구현하는 것은 역부족이다.
                // map이면서 구조를 변경할 수 있는 flatMap을 사용한다.
                // map과 flatMap의 차이점
