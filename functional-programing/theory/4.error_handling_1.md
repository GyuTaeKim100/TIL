...
- 개선 후 예제 7 / None 타입인지 구별하는 함수 생성
    - ```
         ...
         export const isNone = <A>(oa: Option<A>): oa is None<A> => oa._tag === "None"; 
      ``` 
...
## Option 적용 / if문의 합성 / option을 통한 if문 리팩토링
 - 목적
     ...
     - ```
            ...
            const totalPrice = (list: Array<item>) : string => {
                    ...
                    const totalDiscountPrice = totalCalculator(list, (item)=> {
                        ...
                        // landa.js, fp.js를 통해 해당 라이브러리에 구현되어 있는 pipe 또는 compose를 통해 비슷하게 구현이 가능하다.
