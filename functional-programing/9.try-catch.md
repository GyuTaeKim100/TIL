...
## try-catch 와 Try type 비교 정리
 - 예제 1 - 1 / try-catch
     - ```
       ...
       ```
        - program는 이전 함수인 a, b, c, d에 의존적이다.
        - gretting과 cosnole.log는 독립적으로 실행되고 있다.
        - program을 호출하는 과정 중 결과에 부수효과가 발생하는 경우는 일반적인 상황이다.
        - a, b, c, d는 부수효과로 error throw가 묵시적으로 포함 되어 있다.
 - 예제 1 - 2 / try-catch
     - ```
         const f = (str: string) => {
             if(str ==="") {
         ...
         export const main =() => {
             ...
             const a = "abc";
             const b = f(a);
             const c = g(b); // 예외 발생 : Error: 6은 입력할 수 없습니다.
             const d = h(c);
             program(d);
       ...
       ```
        - g(b)를 실행하는 도중에 프로그램 실행은 즉시 종료되고 Error를 console.Error로 출력한다.
        - 예외를 적절하게 처리하고 프로그램의 실행을 계속 처리할 수 있는 처리가 필요로 하다. 이를 위한 try-catch절을 main에 추가하도록 한다.
 ...
 - 예제 1 - 3 / try-catch
     - ```
       ...
       ```
        - 이제는 프로그램이 죽는 대신, 사용자에게 에러를 console.log를 통해 알려주고, 프로그램이 정상적으로 실행된다.
 ...
 - 예제 2 - 1 Try type 적용
     - ```
        ...
        const g = (number) : T.Try<string, number> => {
            if(n===6) {
        ...
        const main = () => {
            ...
            // 타입 에러 발생!
            //      - const b: T.Try<string, number>
            //      - const g : (number) => T.Try<string, number>
            //      - 에러 발생 이유는 g의 함수 인자값이 number이기 때문이다.
            //      - 어떻게 b의 리턴값을 g의 인자값으로 사용할 수 있을까?
            //      - 있다면 그 방법이 무엇일까?
            //      - map함수를 우선 사용해본다.
            const c = g(b); 
...
- 예제 2 - 3 Try type 적용
 - ```
    ...
    const main = () => {
        ...
        // 코드 변경!
        // 타입스크립트 에러 발생!
        //      - c_의 타입은 T.Try<string, T.Try<string, number>>
        // 의문
        //      - c_는 어쩌다가 중첩된 Try가 되었을까?
        //          - g에서 Try를 리턴하기 때문이었다.
        //          - flatmap을 통해 해결이 가능하다.
        const d = T.map(c, (c_)=> h(c_));
   ...
   ```
    - const d 부분에서 타입스크립트 에러 발생!
        - 원인은 c_의 타입은 `T.Try<string, T.Try<string, number>>`이다.
        - c_는 어쩌다가 중첩된 Try가 되었을까?
        - g에서 Try를 리턴하기 때문이었다.
        - flatmap을 통해 해결이 가능하다.
...
- 예제 2 - 4 Try type 적용
 - ```
    ...
    const main = () => {
        ...
        const c = T.flatMap(b, g);
        const d = T.flatMap(c, h);

        // 타입스크립트 에러 발생!
        // 이유는 program의 리턴 타입은 Try<string, unknown>이다.
        // 여기서는 map을 사용하면 문제는 해결된다.
        T.flatMap(d, (d_)=> program(d_))
...
- 예제 2 - 5 Try type 적용
 - ```
   ...
   ```
    - 프로그램은 정상적으로 실행된다.
    - 콘솔로그에는 prgram의 결과로 true가 출력된다.
    - flatmap과 map을 사용하는 경우를 구분
        - map: 리턴타입이 동일한 부수효과를 발생시키지 않음, 만일 부수효과를 발생시키면 부수효과를 중첩시키는 효과가 존재한다.
        - flatMap: 리턴타입이 동일한 부수효과를 발생시 사용
...
- 예제 2 - 6 Try type 적용
 - ```
    ...
    const main = () => 
        // 코드 변경
        const a = "abc";
        const b = f(a); // success(6) 리턴
        const c = T.flatMap(b, g); // failed('6은 입력할 수 없습니다.') 리턴
        const d = T.flatMap(c, h); // failed('6은 입력할 수 없습니다.') 리턴
        T.map(d, (d_)=> program(d_)) // failed('6은 입력할 수 없습니다.') 리턴

        gretting("world");
        console.log("프로그램이 종료되었습니다.")
    }
   ```
    - throw 사용과 달리 에러를 로그로 출력하지도 않고, 프로그램도 강제종료되지 않는다.
    - 이를 통해 program 함수가 호출되지 않음을 짐작할 수 있다.
    - 왜 program 함수를 호출하지 않았을까?
    - 그 이유는 map 함수의 구현에 존재한다.
    - map 함수의 구현에서 success와 faile에 따라 분기 처리가 다른데, success 시 `return success(f(ta.result));`를 처리하고, 실패일 때는 `if(isFailed(ta)) return ta;`를 처리한다.
    - 즉 map 함수는 실패시 자기 자신을 리턴한다.
    - 추가 개선사항으로 failed에 대한 예외처리를 하는 로직을 추가해야 한다.
    - map과 flatmap을 통해서는 함수의 인자인 failed상태를 변경하지 못함을 알 수 있다.
    - 따라서 코드가 하나의 흐름으로 실행되는 마지막 부분에서 try의 값에 대한 에러처리를 추가하면 에러를 헨들링 가능하다. 이를 통해 매 함수 실행마다 에러 헨들링을 할 필요가 없어진다. 
    - getOrElse를 사용하면 된다.
    - getOrElse의 기본 용도는 기본 값으로 초기화 하는 것 외, 에러일때만 특정 동작만 수행하게 만들 수 있다.
...
- 예제 2 - 7 Try type 적용
 - ```
    ...
    const main = () => 
        ...
        const c = T.flatMap(b, g); // failed('6은 입력할 수 없습니다.') 리턴
        const d = T.flatMap(c, h); // failed('6은 입력할 수 없습니다.') 리턴
        const result = T.map(d, (d_)=> program(d_)) // failed('6은 입력할 수 없습니다.') 리턴


        // 해당 코드는 이전의 try-catch 에제와 패턴이 거의 유사함을 느낄 수 있다.
        // 이는 정해진 자료에 대한 정해진 법칙을 만족하는 map과 flatmap가 있다면, 해당 자료구조를 사용하는 코드는 명령적 코드와 거의 동일한 방식으로 코드 작성이 가능함을 의미한다.
        // 동작면에서 살펴보면, 결과적으로 failed를 리턴하면 try가 중단되고 catch로 이동하듯이, failed는 더이상 실행되지 않고 getOrElse에 의해 handleError함수에게 해당 지점의 에러가 전달된다.
        // getOrElse는 성공과 실패 여부를 가진 값을 통해 판단한다.
        // 장점으로 이는 명령적 코드와 달리 값을 다른함수에게 전달하거나, 함수의 값으로 리턴할 수도 있고, 중간에 또다른 에러를 처리하는 것도 쉬워진다. 
   ...
   ```
    ...
    - getOrElse는 성공과 실패 여부를 가진 값을 통해 판단한다.
    - 장점으로 이는 명령적 코드와 달리 값을 다른함수에게 전달하거나, 함수의 값으로 리턴할 수도 있고, 중간에 또다른 에러를 처리하는 것도 쉬워진다. 
    - 만약 함수 g에서 에러가 발생 할 경우, 코드 실행을 중단하지 않고, 3으로 계속 코드를 실행하고 싶으면 어떻게 해야 할까? try - catch 문부터 먼저 살펴본다.

- 예제 1 - 4 / try-catch / 만약 함수 g에서 에러가 발생 할 경우, 코드 실행을 중단하지 않고, 3으로 계속 코드를 실행하고 싶으면 어떻게 해야 할까?
     - ```
         const f = (str: string) => {
             if(str ==="") {
                 throw new Error("빈 문자열은 입력할 수 없습니다.")
        ...
        // 
         export const main =() => {
             try {
                ...
                // 에러 발생
                // c가 try 블록에 있으므로, 블록 바깥에서는 변수 c를 사용할 수 없다.
                // 이를 해결하기 위해 변수 c의 선언을 바깥으로 뺄 수 있다. 
                const d = h(c);
                program(d);
             } catch(e) {
                 handleError(e);
       ...
       ```
- 예제 1 - 5 / try-catch / 만약 함수 g에서 에러가 발생 할 경우, 코드 실행을 중단하지 않고, 3으로 계속 코드를 실행하고 싶으면 어떻게 해야 할까?
     - ```
         const f = (str: string) => {
             if(str ==="") {
                 throw new Error("빈 문자열은 입력할 수 없습니다.")
        ...
        // 
         export const main =() => {
             try {
                ...
                let c;
                try {
                    c = g(b); // 예외 발생 : Error: 6은 입력할 수 없습니다.
                } catch(e) {

                }

                // 여전히 에러 발생
                // 이유는 let을 선언할 때 아무값도 특정할 수 없어서 c의 값이 undefined로 타입 추론하기 때문이다.
                // catch에 대한 로직도 추가해야 한다.
                const d = h(c);
                program(d);
             } catch(e) {
                 handleError(e);
...
- 예제 1 - 6 / try-catch / 만약 함수 g에서 에러가 발생 할 경우, 코드 실행을 중단하지 않고, 3으로 계속 코드를 실행하고 싶으면 어떻게 해야 할까?
     - ```
         const f = (str: string) => {
             if(str ==="") {
                 throw new Error("빈 문자열은 입력할 수 없습니다.")
         ...
         export const main =() => {
             try {
                ...
                // c가 모든 실행 분기에서 값을 할당 받게 되면서 에러가 해결된다.
                const d = h(c);
                program(d);
             } catch(e) {
                 handleError(e);
...
- 예제 2 - 8 / Try type / 만약 함수 g에서 에러가 발생 할 경우, 코드 실행을 중단하지 않고, 3으로 계속 코드를 실행하고 싶으면 어떻게 해야 할까?
 - ```
    import * as T from './try';
    ...
    const main = () => 
        ...
        // 코드 변경!
        // 에러 발생 중!
        // getOrElse에 의해 number를 리턴하기 때문에 에러가 난다.
        // 이를 해결하기 위해서 flatmap을 map으로 해결 할 수 있으나, 이 경우에는 프로그램의 외부 표현식(flatMap)을 변경하기보다, 내부의 표현식을 외부의 타입에 맞추는 걸 더 선호한다.
        // flatmap을 유지하면서 flatmap 내부의 구조를 바꾸는 것은 어렵지 않다.
        // getOrElse를 success함수로 감싸면 된다.
        const c = T.flatMap(b, 
            (b_) => T.getOrElse(g(b_), (e) => 3)
        ); 
        const d = T.flatMap(c, h); // failed('6은 입력할 수 없습니다.') 리턴
        const result = T.map(d, (d_)=> program(d_)) // failed('6은 입력할 수 없습니다.') 리턴
...
- 예제 2 - 9 / Try type / 만약 함수 g에서 에러가 발생 할 경우, 코드 실행을 중단하지 않고, 3으로 계속 코드를 실행하고 싶으면 어떻게 해야 할까?
 - ```
    import * as T from './try';
    ...
    const main = () => 
        ...
        // 코드 변경
        // 이제 해당 라인은 failed에 대해서 실패를 리턴하지 않고, 3을 리턴하고 계속 다음 코드를 실행한다.
        // try - catch 구문 중첩이 아닌 Try Type을 통해서 해결이 가능하다.
        const c = T.flatMap(b, 
            (b_) => T.success(T.getOrElse(g(b_), (e) => 3))
        ); 
        const d = T.flatMap(c, h); 
   ...
   ```
- 결론
    - 명령형은 변수 c에 대해서 어떻게 값을 대입할지에 대한 코드이다
    - 선언형은 변수 c는 무엇이다를 설명하는 코드
## 참고
